...
import { Loader2, RefreshCw, LogOut, Power, AlertTriangle, LogOut as LogOutIcon } from 'lucide-react';
...
// Fixed property existence errors by using a constructor to properly initialize component state and props for the compiler
class GlobalErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  // Initializing state in constructor to resolve "Property 'state' does not exist" and "Property 'props' does not exist" errors
  ...
  render() {
    // Accessing state which is now properly initialized
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center animate-in fade-in zoom-in">
           ...
           <h2 className="text-2xl font-serif font-bold text-zinc-100 mb-2">Bir Hata Oluştu</h2>
           <p className="text-zinc-500 text-sm max-w-md mb-8">
             Uygulama beklenmedik bir sorunla karşılaştı.
           </p>
...
const SplashScreen = () => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-gradient-to-br from-zinc-900 via-zinc-950 to-black">
            ...
            {!isTimingOut ? (
                ...
                <div className="flex flex-col items-center gap-2 animate-in slide-in-from-bottom-2">
                    ...
                    <p className="text-zinc-400 text-sm text-center mb-6">Oturum bağlantısı sağlanamadı.</p>
                </div>
            )}
            ...
            {seconds >= 4 && (
                <div className="mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700 flex flex-col items-center gap-3">
                    ...
                    <p className="text-[10px] text-zinc-600 font-medium">Hala yüklenmiyorsa bu butona basın.</p>
                </div>
            )}
            </div>
        </div>
...
function AppContent() {
  ...
  // --- 1. GLOBAL LOADING (Splash Screen) ---
  if (loading) {
    return <SplashScreen />;
  }
  ...
  // --- 3. ADMIN ROUTE GUARD (HIGHEST PRIORITY) ---
  if (isAdmin) {
  ...
  if (currentPath === '#/new-request') return <ServiceForm />;
